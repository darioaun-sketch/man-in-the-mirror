...
<div class="header">
  <div>
    ...
    <div class="top-btns">
      <button class="icon-btn" id="btn-theme" title="Modo Oscuro/Claro">🌙</button>
      <button class="icon-btn" id="btn-future" title="Tú del Futuro">🚪</button>
    </div>
  </div>
...
<div class="modal-overlay" id="modal-future">
  <div class="modal-box">
    <div class="card-label">Proyección a 1 Año 🚪</div>
...
function renderHabitList(di) {
  ...
  return AREAS.map(a => {
    ...
    HABITOS[a.k].forEach(h => {
      ...
      let timerBtn = TIMERS[h] && !hd.done ? `<button class="btn-timer" onclick="openTimer('${safeH}', ${di}, ${TIMERS[h]})">⏲️</button>` : '';
      ...
      <div class="habit-row-wrap">
        ...
        <input type="text" class="text-input habit-note" data-di="${di}" data-h="${safeHInput}" value="${hd.note||''}" placeholder="Notas / Detalles...">
      </div>`;
    });
    return html;
  }).join('');
...
document.getElementById('btn-future').addEventListener('click', () => {
  ...
  msg += `<b>🏋️ ${projGym} días de entrenamiento</b><br><br>`;
